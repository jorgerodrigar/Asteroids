...
void GunInputComponent::handleInput(GameObject* o, Uint32 time, const SDL_Event& event) {
	if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_SPACE && actualShots < maxShots && timeSinceLastShot < 3000) {
		if(!tiempoIniciado)timeSinceLastUpdate = SDL_GetTicks();
		tiempoIniciado = true;
		Message msg{ FIGHTER_SHOOT };
		send(&msg);
		actualShots++;
	}
	if (tiempoIniciado) timeSinceLastShot = SDL_GetTicks() - timeSinceLastUpdate;
